...
date_val = str(datetime.now())[0:10]

#File to write results to, new report generated daily, set filepath as appropriate
my_stocks_csv = open("robinbot_results - " + str(date_val) + ".csv", "w+")

user= profile.user
passw= profile.passw

my_stocks_csv.write("Ticker,Name,Price,Quantity,Average Buy Price,Equity,Equity Change,Average Buy Price\n") # For Excel
#my_stocks_csv.write("Ticker Name Price Quantity Average_Buy_Price Equity Equity_Change Average_Buy_Price\n") # For OpenOffice Calc

mfa_code = profile.mfa # MFA INFO
...
print("The net equity change is {:.2f}".format(total_equity_change))
